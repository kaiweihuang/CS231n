...
class PretrainedCNN(object):
  def __init__(self, dtype=np.float32, num_classes=100, input_size=64, h5_file=None):
    ...
    for i, (f, next_dim) in enumerate(zip(self.filter_sizes, self.num_filters)):
      fan_in = f * f * prev_dim
      self.params['W'+str(i + 1)] = np.sqrt(2 / fan_in) * np.random.randn(next_dim, prev_dim, f, f)
    ...
    fan_in = int(cur_size) * int(cur_size) * self.num_filters[-1]
    self.params['W'+str(i + 2)] = np.sqrt(2 / fan_in) * np.random.randn(fan_in, hidden_dim)
    self.params['b'+str(i + 2)] = np.zeros(hidden_dim)
    self.params['gamma'+str(i + 2)] = np.ones(hidden_dim)
    self.params['beta'+str(i + 2)] = np.zeros(hidden_dim)
    self.bn_params.append({'mode': 'train'})
    self.params['W'+str(i + 3)] = np.sqrt(2 / hidden_dim) * np.random.randn(hidden_dim, num_classes)
