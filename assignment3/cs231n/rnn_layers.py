...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  cache = (x,affine_output,next_h,prev_h,Wx,Wh)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  x,affine_output,next_h,prev_h,Wx,Wh = cache
  daffine_output = dnext_h*(1-next_h**2)
...
def rnn_backward(dh, cache):
  ...
  ##############################################################################
  N,T,H = dh.shape
  latest_x,_,_,_,latest_Wx,latest_Wh = cache[T-1]
  _,D = latest_x.shape
  dx = np.zeros((N,T,D))
  dprev_h = np.zeros((N,H))
  dWx = np.zeros(latest_Wx.shape)
  dWh = np.zeros(latest_Wh.shape)
  db = np.zeros((H))
  for t in range(T-1,-1,-1):
      dx[:,t,:],dprev_h,dWx_temp,dWh_temp,db_temp = rnn_step_backward(dh[:,t,:]+dprev_h,cache[t]) # Do not forget dprev_h
      dWx += dWx_temp
      dWh += dWh_temp
      db += db_temp
  dh0 = dprev_h
...
def word_embedding_backward(dout, cache):
  ...
  np.add.at(dW,x,dout)
  '''
  This code is a bit hard to understand at the beginning, which is equivalent to the following operations:
  for n in range(N):
   for t in range(T):
      dW[x[n,t],:] += dout[n,t,:]
  '''
...
def lstm_backward(dh, cache):
  ...
  #############################################################################
  N,T,H = dh.shape
  x,prev_h,prev_c,Wx,Wh,i,f,o,g,next_c,next_h = cache[T-1]
  _,D = x.shape
  dx = np.zeros((N,T,D))
  ...
  db = np.zeros((4*H))
  for t in range(T-1,-1,-1):
      dx[:,t,:],dprev_h,dprev_c,dWx_temp,dWh_temp,db_temp = lstm_step_backward(dh[:,t,:]+dprev_h,dprev_c,cache[t])
      dWx += dWx_temp
      dWh += dWh_temp
      db += db_temp
  dh0 = dprev_h
